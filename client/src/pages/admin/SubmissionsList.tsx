...
export function SubmissionsList() {
  ...
  const handleDownloadPdf = async (sub: Submission) => {
    ...
    wrapper.innerHTML = `
      <div style="font-family: 'Georgia', serif; color: #111; max-width: 720px; margin: 0 auto; padding: 0">
        <div style="background:#0f3261; color:white; padding:32px 40px; margin-bottom:32px">
  ...
  const getStatusBadge = (sub: Submission) => {
    ...
    if (sub.marksAwarded !== undefined && sub.marksAwarded !== null) return <span className="text-[10px] font-bold uppercase tracking-widest px-2 py-1 rounded bg-[#00ff87]/10 border border-[#00ff87]/30 text-[#00ff87]">Marked</span>;
    return <span className="text-[10px] font-bold uppercase tracking-widest px-2 py-1 rounded bg-[#f5a623]/10 border border-[#f5a623]/30 text-[#f5a623]">Needs Marking</span>;
  };
    ...
    <div className="max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500 h-full flex flex-col">
      <div className="flex items-center justify-between shrink-0">
        <div>
          ...
          <p className="text-[#00ff87] text-sm tracking-widest uppercase mt-1">{exam.title}</p>
        </div>
        <div className="text-right bg-[#1a4478] border border-white/5 px-6 py-4 rounded-xl relative overflow-hidden">
          <div className="absolute right-0 top-0 w-16 h-16 bg-[#00ff87] rounded-full mix-blend-screen filter blur-[30px] opacity-20 pointer-events-none"></div>
          <p className="text-[10px] text-gray-400 uppercase font-bold tracking-[0.2em] mb-1">Total Marks</p>
          <p className="text-3xl font-black text-[#00ff87]">{totalPossibleMarks}</p>
        </div>
      </div>
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-3 gap-8 min-h-0">
        ...
        <div className="bg-[#1a4478] border border-white/5 rounded-xl overflow-hidden flex flex-col h-[calc(100vh-250px)] shadow-xl relative group">
          <div className="absolute top-0 right-0 w-32 h-32 bg-[#fe0979] rounded-full mix-blend-screen filter blur-[80px] opacity-10 group-hover:opacity-20 transition-opacity pointer-events-none"></div>
          <div className="p-6 border-b border-white/5 bg-[#0f3261]/50 relative z-10 shrink-0">
            <h3 className="text-[12px] tracking-widest uppercase text-[#fe0979] font-bold">Student Submissions <span className="ml-2 text-white bg-white/10 px-2 py-0.5 rounded">{submissions.length}</span></h3>
          </div>
                  ...
                  <li 
                    ...
                    className={`p-5 cursor-pointer transition-all duration-300 relative overflow-hidden ${
                      selectedSubmission?.id === sub.id 
                        ? 'bg-white/5 border-l-4 border-l-[#fe0979]' 
                        : 'hover:bg-white/5 border-l-4 border-l-transparent'
                    }`}
                  >
                    {selectedSubmission?.id === sub.id && (
                       <div className="absolute left-0 top-0 bottom-0 w-32 bg-gradient-to-r from-[#fe0979]/10 to-transparent pointer-events-none"></div>
                    )}
                    ...
                    <div className="flex justify-between items-center text-xs mt-3 relative z-10">
                      ...
                      {sub.marksAwarded != null && (
                        <p className="font-bold text-[#00ff87] tracking-widest">{sub.marksAwarded} / {totalPossibleMarks}</p>
                      )}
                    </div>
                  </li>
                ))}
        ...
        <div className="lg:col-span-2 h-[calc(100vh-250px)]">
          {!selectedSubmission ? (
            <div className="bg-[#1a4478] border border-white/5 border-dashed rounded-xl h-full flex flex-col items-center justify-center p-12 text-center">
            ...
            <div className="bg-[#1a4478] border border-white/5 rounded-xl h-full flex flex-col shadow-xl overflow-hidden relative group">
              <div className="absolute top-0 right-0 w-48 h-48 bg-[#00f2fe] rounded-full mix-blend-screen filter blur-[100px] opacity-10 pointer-events-none"></div>
              
              <div className="p-6 border-b border-white/5 bg-[#0f3261]/50 shrink-0 relative z-10 flex justify-between items-center">
                <div>
                ...
                <div className="flex items-center gap-3">
                  <button
                    ...
                    className="flex items-center gap-2 px-4 py-2 border border-[#00f2fe]/30 hover:bg-[#00f2fe]/10 text-[#00f2fe] rounded-lg text-xs font-bold tracking-widest uppercase transition-all"
                  >
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-sm font-mono text-[#00f2fe]">
              ...
              <div className="p-8 overflow-y-auto flex-1 space-y-8 custom-scrollbar bg-[#0f3261] relative z-10">
                {selectedSubmission.answers?.freeform ? (
                  ...
                  questions.map((q, idx) => (
                    <div key={q.id} className="bg-[#1a4478] p-6 rounded-xl border border-white/5 shadow-lg relative overflow-hidden">
                      <div className="absolute left-0 top-0 bottom-0 w-1 bg-[#00f2fe]/50" />
                      <div className="flex justify-between items-start mb-4">
                        <div className="font-medium text-white max-w-[80%] leading-relaxed text-sm">
                          <span className="font-black text-[#00f2fe] mr-3 text-lg tracking-wider">Q{idx + 1}.</span>
                          {q.prompt}
                        </div>
                        <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest bg-[#0f3261] border border-white/10 px-3 py-1.5 rounded">
                          {q.marks} Mark{q.marks !== 1 ? 's' : ''}
                        </span>
                      </div>
                      <div className="mt-4 p-4 bg-[#00f2fe]/5 rounded-lg border border-[#00f2fe]/20">
                        <p className="text-[10px] font-bold text-[#00f2fe] uppercase tracking-widest mb-3">Student's Answer</p>
              ...
              <div className="p-6 border-t border-white/5 bg-[#0f3261]/80 shrink-0 relative z-10 backdrop-blur-md">
                <form onSubmit={handleMarkSubmission} className="flex gap-6 items-start">
                  <div className="w-32">
                    ...
                    <input
                      ...
                      className="w-full bg-[#1a4478] text-[#00ff87] font-black text-center border border-[#00ff87]/30 rounded-lg p-3 focus:outline-none focus:border-[#00ff87] focus:ring-1 focus:ring-[#00ff87] transition-colors"
                    />
                  </div>
                  <div className="flex-1">
                    ...
                    <textarea
                      id="note"
                      className="w-full bg-[#1a4478] text-white placeholder-gray-600 border border-white/10 rounded-lg p-3 text-sm focus:outline-none focus:border-[#00f2fe] focus:ring-1 focus:ring-[#00f2fe] transition-colors resize-none"
                      rows={2}
                  ...
                  <div className="pt-6">
                    <button 
                      ...
                      className="px-8 py-3 bg-[#00ff87] hover:bg-[#00d671] text-[#0f3261] rounded-lg text-xs font-bold tracking-[0.2em] uppercase transition-all shadow-[0_0_15px_rgba(0,255,135,0.4)] disabled:opacity-50 disabled:shadow-none whitespace-nowrap"
                    >
                ...
                {!selectedSubmission.isFinal && (
                  <p className="text-[10px] tracking-widest uppercase text-[#f5a623] mt-4 font-bold flex items-center gap-2">
